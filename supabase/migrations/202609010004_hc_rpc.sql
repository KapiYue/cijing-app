...
--
-- 依赖：packages/db/migrations/cijing/202609010002_review_idempotency.sql 必须**先**跑
--       （hc_apply_review 要用到带 p_client_event_id 的新签名）。

-- ─────────────────────────────────────────────────────────────
-- 今日队列。`design.md` §5.4 S4
--
--   今日队列 = 到期复习卡（按 due 升序）
...
--
-- 「capture > 词包」这条优先级**必须在排序代码里体现，不是文案**（§5.4 S4 的红字）。
...
--
-- 为什么包一层：§8.3 的 activated_at「一次写入永不改写」是跨三端的不变量，
...
begin
  ...
  -- ⚠️ 阈值 21 写死在这里是有意的——它只能升不能降，且改阈值时已落库的不重算（§8.2）。
...
-- ─────────────────────────────────────────────────────────────
-- 首页概览。§5.4 S4 的「7 个词需要复习 / 13 个新词 / 连续 6 天 / 已经激活 83 个活词」
--
...
-- ─────────────────────────────────────────────────────────────
-- 埋点批量上报。§9.5：攒批 + 一轮学习结束时一次性 flush，**不单独发请求**。
-- ─────────────────────────────────────────────────────────────
